...
#include CYGBLD_HAL_BOARD_H		// board specifics configuartion
...
#include CYGBLD_HAL_VAR_H		// platform definitions
...
static void
cyg_hal_plf_serial_write(void *__ch_data, const cyg_uint8 *__buf, cyg_uint32 __len)
{
	CYGARC_HAL_SAVE_GP();
	while (__len-- > 0) {
...
static int
cyg_hal_plf_serial_isr(void *__ch_data, int *__ctrlc, CYG_ADDRWORD __vector, CYG_ADDRWORD __data)
{
	int		res = 0;
	channel_data_t	*chan = (channel_data_t *) __ch_data;
	char		c;
...
cyg_hal_plf_serial_init(void)
{
	...
	// Disable interrupts.
	HAL_INTERRUPT_MASK(ser_channels[0].isr_vector);
	HAL_INTERRUPT_MASK(ser_channels[1].isr_vector);

	// Unmask UART0/1 RX interrupt
	HAL_WRITE_UINT32(INTSUBMSK, __READ_UINT32(INTSUBMSK) & ~(BIT_SUB_RXD0|BIT_SUB_RXD1));

	// Init channels
	cyg_hal_plf_serial_init_channel(&ser_channels[0]);
	cyg_hal_plf_serial_init_channel(&ser_channels[1]);
